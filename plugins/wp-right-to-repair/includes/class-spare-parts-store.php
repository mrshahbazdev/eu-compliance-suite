<?php
/**
 * Spare-parts supplier directory.
 *
 * Table: wp_eurocomply_r2r_suppliers
 *
...
final class SparePartsStore {

	public const DB_VERSION_OPTION = 'eurocomply_r2r_suppliers_db_version';
	public const DB_VERSION        = '1.0.0';

	public static function table_name() : string {
		global $wpdb;
		return $wpdb->prefix . 'eurocomply_r2r_suppliers';
	}
	...
	public static function install() : void {
		...
		$sql = "CREATE TABLE {$table} (
			...
			availability_years INT UNSIGNED NOT NULL DEFAULT 0,
			notes LONGTEXT NULL,
			PRIMARY KEY  (id),
	...
	public static function all( string $category = '', string $country = '', int $limit = 500 ) : array {
		...
		$args[] = $limit;
		$sql    = $wpdb->prepare( "SELECT * FROM {$table} {$where} ORDER BY name ASC LIMIT %d", $args ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private static function normalise( array $data ) : array {
		...
		return array(
			...
			'availability_years' => max( 0, min( 15, (int) ( $data['availability_years'] ?? 0 ) ) ),
